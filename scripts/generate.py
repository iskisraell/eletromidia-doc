#!/usr/bin/env python3
"""Eletromidia branded document generator. Accepts JSON content, outputs DOCX + PDF."""
...
FOOTER_IMG = os.path.join(ASSETS_DIR, "footer-logo.png")

# Eletromidia brand colors
ORANGE = (255, 79, 0)
BLACK = (0, 0, 0)
DARK_GRAY = (51, 51, 51)
WHITE = (255, 255, 255)
...
# PDF generator (fpdf2)
# ===================================================================
class EletromidiaPDF:
    def __init__(self):
        from fpdf import FPDF

        self.pdf = FPDF("P", "mm", "A4")
        self.pdf.set_auto_page_break(auto=True, margin=25)
        # Montserrat TTF: full Unicode including Portuguese diacritics (ç, ã, é, ê, â, ó, ú, í)
        ...
        # Replace header / footer with branded versions
        _pdf = self.pdf

        def _header():
            if os.path.exists(HEADER_IMG):
                _pdf.image(HEADER_IMG, x=19, y=12, w=48, h=14)
            _pdf.set_y(32)

        def _footer():
            if os.path.exists(FOOTER_IMG):
                _pdf.image(FOOTER_IMG, x=(PAGE_W - 5.2) / 2, y=284, w=5.2, h=9.4)
            _pdf.set_y(-10)
    ...
    def _section_title(self, text):
        p = self.pdf
        # Smart page break: only start a new page if <45mm remains
        if p.get_y() > 220:
            p.add_page()
        p.ln(6)
    ...
    def _bullet(self, text, level=0):
        ...
        x = p.get_x()
        p.set_x(x + indent)
        p.multi_cell(190 - indent, 5, f"{marker}  {text}")
        p.ln(1)
    ...
    def _add_table(self, headers, rows, col_widths=None):
        ...
        for row_idx, row in enumerate(rows):
            ...
            y_row = p.get_y()
            p.rect(10, y_row, sum(col_widths), row_h, style="F")
            for i in range(len(headers)):
                p.rect(10 + sum(col_widths[:i]), y_row, col_widths[i], row_h, style="D")
            for i, cell in enumerate(row):
                p.set_xy(10 + sum(col_widths[:i]) + 2, y_row + 1)
    ...
    def build(self, content: dict) -> "fpdf.FPDF":
        ...
        p.add_page()
        p.set_y(45)
        p.set_font("Montserrat", "B", 28)
        p.set_text_color(*ORANGE)
        p.cell(0, 14, content["title"], align="C", new_x="LMARGIN", new_y="NEXT")
        p.ln(2)

        for line in content.get("subtitle", []):
            p.set_font("Montserrat", "B", 22)
            p.set_text_color(*BLACK)
            p.cell(0, 12, line, align="C", new_x="LMARGIN", new_y="NEXT")
        p.ln(8)
        ...
        if note:
            ...
            p.set_text_color(*DARK_GRAY)
            p.cell(0, 8, note, align="C", new_x="LMARGIN", new_y="NEXT")
        p.ln(16)
...
class EletromidiaDOCX:
    ...
    def _set_paragraph_font(self, para, size=9, bold=False, italic=False, color=DARK_GRAY, align=None):
    ...
    def _section_title(self, text):
        ...
        run.font.bold = True
        run.font.color.rgb = self.RGBColor(*ORANGE)
        para.space_after = self.Pt(6)
        # Add orange border-bottom via paragraph formatting
    ...
    def build(self, content: dict):
        ...
        for m in content.get("meta", []):
            ...
            run_value = para.add_run(m["value"])
            run_value.font.name = "Montserrat"
            run_value.font.size = self.Pt(9)

        # Sections — page break only after cover (first section), then spacing
...
def main():
    ...
    base_name = content.get("filename", "Eletromidia_Document")

    results = []

    if do_pdf:
        pdf_builder = EletromidiaPDF()
